...
fn build_shebang_argv(
    ...
    // Linux passes: interpreter, optional shebang argument, script filename,
    // then the caller's arguments after argv[0]. The old implementation kept
    // the caller's argv[0] instead of the script filename, which made apk's
    // relative post-install and trigger scripts fail as `sh: can't open`.
